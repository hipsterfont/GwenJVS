...
void Input::SetupJVS() {
	...
	if (!GetCommState(com_port_handle_, &dcb)) {
		if (log_error_)
			LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"SetupJVS - GetCommState failed with error %d.\r\n", GetLastError()));
		throw;
	}
	...
	LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"BaudRate = %d, ByteSize = %d, Parity = %d, StopBits = %d\r\n", dcb.BaudRate, dcb.ByteSize, dcb.Parity, dcb.StopBits));
...
bool Input::Initialize() {
	...
	if (!ClearCommError(com_port_handle_, NULL, 0)) {
		if (log_error_)
			LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"ClearCommError()!\r\n"));
		return false;
	}
...
bool Input::ReadJVS(uint8_t* buffer, int size) {
	...
	if (!os.hEvent) {
		...
		if (log_error_)
			LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"ReadJVS - CreateEvent() error 0x%x!\r\n", error));
	...
	if (!WaitCommEvent(com_port_handle_, &event, &os)) {
		DWORD error = GetLastError();
		if ((error != ERROR_IO_PENDING) || (os.hEvent == 0)) {
			if (log_error_)
				LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"ReadJVS - WaitCommEvent() error 0x%x!\r\n", error));
			CloseHandle(os.hEvent);
			return false;
		}
		else {
			...
			switch (result) {
				case WAIT_TIMEOUT: {
					if (log_error_)
						LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"ReadJVS - WaitCommEvent() timeout exceeded!\r\n"));
					CloseHandle(os.hEvent);
					return false;
					break;
				}
		...
		if (raw_frame[0] == static_cast<uint8_t>(JVS_OPCODES_SYNC)) {
			if (raw_frame[1] == 0) { // destination will always be 0
				...
				if (received_size > size) {
					if (log_error_)
						LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"ReadJVS - Payload too large for buffer!\r\n"));
					return false;
				}
				int i = 3;
				int j = 0;
				...
				if (j != received_size) {
					if (log_error_)
						LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"ReadJVS - Incorrect payload length received!\r\n"));
					return false;
				}
	...
	if (log_error_)
		LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"ReadJVS - Incomplete frame received!\r\n"));
	return false;
}
...
bool Input::WriteJVS(uint8_t dest, const uint8_t* data, int size) {
	...
	for (auto i = 0; i < size; ++i) {
		if (j > sizeof(raw_frame)) {
			if (log_error_)
				LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"WriteJVS - Payload too large for buffer!\r\n"));
			return false;
		}
...
#ifdef DEBUG
	LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"WriteJVS - Raw frame:\r\n\t"));
	memset(ltext, 0, sizeof(ltext));
	...
	if (!WriteFile(com_port_handle_, &raw_frame, j, &bytes_sent, &os)) {
		if (GetLastError() != ERROR_IO_PENDING) {
			if (log_error_)
				LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"WriteJVS - Write Error!\r\n"));
			CloseHandle(os.hEvent);
			return false;
		}
		DWORD result = WaitForSingleObject(os.hEvent, kReadTimeout);
		switch (result) {
			case WAIT_TIMEOUT: {
				if (log_error_)
					LogEvent(log_file_handle_, ltext, swprintf_s(ltext, sizeof(ltext) / sizeof(wchar_t), L"WriteJVS - Timeout exceeded!\r\n"));
				CloseHandle(os.hEvent);
				return false;
				break;
			}
